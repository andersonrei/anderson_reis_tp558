...
# Normalização preservando a parte complexa
encoded_symbol_normalizing = tf.sqrt(tf.reduce_mean(tf.square(tf.abs(encoded_symbol_iffted))))
encoded_symbol_original = encoded_symbol_iffted / tf.cast(encoded_symbol_normalizing, tf.complex64)
...
for SNR_range in np.arange(15, 16, 5):
    ...
    # Apaga a pasta apenas se ela existir
    ...
    PAPR_dB = 10 * np.log10(PAPR_sample)


    # Garante que a pasta 'result' existe
    os.makedirs("./result", exist_ok=True)

    # Salva os valores da PAPR em um arquivo de texto
    np.savetxt("./result/PAPR_dB_{0}dB.txt".format(SNR_range), PAPR_dB)
    ...
    folder_path = "./result"
    file_path = os.path.join(folder_path, "CCDF")
    # Cria a pasta se ela não existir
    os.makedirs(folder_path, exist_ok=True)
    # Salva o array no arquivo
    np.savetxt(file_path, CCDF)
    #np.savetxt("./result/CCDF_trained_at_{0}dB SNR_L4_64_rly".format(SNR_range), CCDF)
    # Define o caminho da pasta e do arquivo
    folder_path = "./result"
    file_path = os.path.join(folder_path, "PAPR_sample")
    # Cria a pasta se ela não existir
    os.makedirs(folder_path, exist_ok=True)
    # Salva o array no arquivo
    np.savetxt(file_path, PAPR_sample)
...
# Define o caminho da pasta e do arquivo
folder_path = "./result"
file_path = os.path.join(folder_path, "cost_plot")

# Cria a pasta se ela não existir
os.makedirs(folder_path, exist_ok=True)

# Salva o array no arquivo
np.savetxt(file_path, cost_plot)

PAPR_dB = np.loadtxt("./result/PAPR_dB_{0}dB.txt")
z = np.arange(0, 6, 0.2)
CCDF = [np.sum(PAPR_dB > v) / len(PAPR_dB) for v in z]

plt.semilogy(z, CCDF)
plt.xlabel("PAPR [dB]")
plt.ylabel("CCDF (Prob[PAPR > z])")
plt.title("CCDF da PAPR")
